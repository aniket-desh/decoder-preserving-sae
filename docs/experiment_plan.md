...
Full BatchTopK codes are passed to sklearn as exact-value SciPy CSR matrices, exposing their structural zeros without changing any value. Original-residual and reconstruction companion probes call pinned upstream `find_best_reg` sequentially. Full-code companion probes preserve its ten C values, CV splits, fresh cold `LogisticRegression` per C and fold, `lbfgs` objective, seed, tolerance, iteration limit, original-order `np.argmax` tie rule, and cold selected-C refit on the exact seeded full-train shuffle; the ten independent C evaluations are dispatched with at most eight process jobs per worker and nested numerical-library threads capped at one. The simultaneous smoke therefore exercises up to 32 cold-C processes under the pod's 32.3-core cgroup quota, including the parent and orchestration contention that invalidated the isolated schema-v5 estimate, while each sequential upstream probe is capped at eight threads. Each timing worker is projected separately using its own sparse method and frozen 3/3/2/2 companion-seed count. The authoritative wall-time projection adds the measured cold-cache duration and the maximum pre-barrier initialization once to the slowest worker's p95 workload with 30% headroom; rows are never pooled or averaged across workers. Concept workers are gated on a passed schema-v6 report at or below three pod-hours, and every superseded or isolated runtime schema is rejected even if its other fields pass.
