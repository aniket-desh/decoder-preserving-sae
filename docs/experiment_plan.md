...
Batch preparation writes a hash-bound spend ledger before submission. The ledger reserves the frozen maximum for primary labels, nano cleanup, held-out evaluation, and hard-case adjudication together; submission, polling, download, and finalization all fail closed if the aggregate worst-case total exceeds $10 or any reservation, manifest, request, mapping, or config identity drifts. The conditional Batch lifecycle implementation is preserved in the immutable closure source bundle rather than the public release tree because the concept gate blocked every API call. Its poller was designed to resume an existing Batch ID without resubmitting, persist every remote status, accept only a completed zero-failure request count, validate every custom ID and structured label, and reuse a hash-verified result after interruption.
...
Suggested session names are `dpsae-concept-timing`, `dpsae-concept-pilot`, `dpsae-frozen-confirm`, `dpsae-spectral-screen`, `dpsae-concept-confirm-s0`, `dpsae-concept-confirm-s1`, and `dpsae-concept-confirm-s2`. Every long command must tee to a stable log path inside its stage output directory. The archived resource watcher records all four GPUs, host RAM, `/workspace` capacity, kernel OOM-event availability/count, load, and active tmux sessions every 30 seconds. Warnings begin at 44,000 MiB GPU memory, 88 C, 12 GiB available host RAM, or 25 GiB free storage, with storage marked critical at 10 GiB. This provider-specific watcher remains in the immutable closure source bundle rather than the public release tree.
...
**July 17, 2026, figure typography retrofit.** The publication scale increased to 8.25 pt base and axis labels, 7.75 pt ticks and legends, and a 7.5 pt minimum for compact annotations. The six live TikZ/PGFPlots figures are migrated from inherited manuscript fonts to the same pinned D-DIN Regular, Italic, and Bold files used by Matplotlib, without changing body or caption typography. Whole-figure resize wrappers are removed so the size contract is evaluated at the final 5.50-inch layout, and every regenerated closure figure records both the Python style layer and the exact `.mplstyle` hash.

**July 17, 2026, public code release consolidation.** The public tree retains every executed scientific experiment, frozen config, reported negative-result runner, artifact auditor, aggregation step, and figure renderer. Provider-specific backup, monitoring, queue, and retry helpers are removed from the public surface, as are conditional Exp12/Exp13, context-mining, and API-labeling implementations that the Exp10 gate never authorized. The immutable closure source bundle and recorded experiment revisions remain the byte-exact audit target for the executed fleet; the cleaned tree is the reader-facing implementation and reproduction surface.
